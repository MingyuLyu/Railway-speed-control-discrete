...
def main():
	...
	if opt.write:
		...
		writepath = 'runs/SACD_{}'.format('TrainSpeedControl') + timenow
	...
	if opt.render:
		while True:
			...
			print('EnvName:', 'TrainSpeedControl', 'seed:', opt.seed, 'score:', score)
	else:
		total_steps = 0
		while total_steps < opt.Max_train_steps:
			...
			while not done:
				...
				if total_steps % opt.eval_interval == 0:
					...
					print('EnvName:','TrainSpeedControl', 'seed:', opt.seed,
						  'steps: {}k'.format(int(total_steps / 1000)), 'score:', int(score))
				total_steps += 1

				'''save model'''
				if total_steps % opt.save_interval == 0:
					agent.save(int(total_steps/1000), 'TrainSpeedControl')
	env.close()
	eval_env.close()
